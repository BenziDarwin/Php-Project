...
<div class="flex items-center">
  <div class="w-1/3">
    ...
    @if (isset($userProfileImage) && $userProfileImage->userProfileImage)
        <img id="profile-image" src="{{ $userProfileImage->userProfileImage }}" class="rounded-full w-full h-full object-cover">
    @else
        <img id="profile-image" src="{{ isset($userProfileImage) && $userProfileImage->userProfileImage ? $userProfileImage->userProfileImage : 'https://via.placeholder.com/150' }}" alt="User Profile Image" class="rounded-full w-full h-full object-cover">
    @endif
</span>
          ...
          <input type="file" name="image" id="image" class="hidden" onchange="document.getElementById('profile-image').src = window.URL.createObjectURL(this.files[0])">
        </label>
      </div>
    </form>
  </div>
        ...
        <div class="flex flex-col justify-center items-center h-[60vh]">
            <div class="relative flex flex-col items-center rounded-[20px] w-[700px] max-w-[95%] mx-auto bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:!shadow-none p-3">
                <div class="grid grid-cols-2 gap-4 px-2 w-full">
                    ...
                    <div class="flex flex-col items-start justify-center rounded-2xl bg-white bg-clip-border px-3 py-4 shadow-3xl shadow-shadow-500 dark:!bg-navy-700 dark:shadow-none">
                    <p class="text-sm text-gray-600 dark:text-black">Course</p>
                    <p class="text-base font-medium text-navy-700 dark:text-black">
                        {{$user->course}}
                    </p>
                    </div>
